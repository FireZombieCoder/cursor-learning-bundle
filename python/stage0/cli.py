...
def info(contract_address, abi_path):
    ...
    try:
        ...
        network_name = (
            "Base-Sepolia"
            if info["chain_id"] == 84532
            else "Base Mainnet"
            if info["chain_id"] == 8453
            else f"Unknown ({info['chain_id']})"
        )
...
def events(contract_address, abi_path, count):
    ...
    try:
        ...
        # Create events table
        table = Table(
            title=f"📜 Recent Events (last {min(count, len(events))})",
            show_header=True,
        ...
        for event in events[-count:]:
            ...
            table.add_row(str(event["blockNumber"]), message, updater_short, tx_short)
...
def balance(contract_address, abi_path):
    ...
    try:
        ...
        panel = Panel(balance_text, title="💰 Account Balance", border_style="green")
